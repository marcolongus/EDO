...
Frente a un problema de este tipo aparecen, al menos, tres preguntas diferentes:
	...
	La idea que organiza esta clase es estudiar primero cuál es la máxima información que podemos extraer de una ecuación diferencial \emph{antes de resolverla}. Analizaremos su dominio, la existencia y unicidad de las soluciones, sus posibles formas de representación y la estabilidad de los equilibrios. Recién después introduciremos la separación de variables como método analítico para obtener soluciones. Los campos de direcciones, el estudio general de las ecuaciones autónomas y la integración directa se desarrollan en el material suplementario de esta clase.
...
mientras que el signo de $y_0$ selecciona la rama correspondiente.
	...
	Para estudiar la estabilidad nos restringiremos a ecuaciones autónomas de la forma

	\[
	\dot{y}=f(y).
	\]
Un valor \(y_\ast\) es un punto de equilibrio si \(f(y_\ast)=0\), pues entonces la función constante \(y(x)=y_\ast\) es solución. El signo de \(f(y)\) a cada lado de los equilibrios determina el sentido de evolución y puede representarse mediante una recta de fase. El desarrollo detallado de estas ideas se encuentra en el material suplementario de la Clase 2.
...
en cualquier intervalo donde el denominador no se anule. La solución de equilibrio \(y=1\) corresponde a \(C=0\), mientras que \(y=0\) debe agregarse por separado.
	...
	\begin{quote}
		\textbf{Relación con la Guía 1.} Los problemas 18--20 practican separación de variables y PVI. El caso de integración directa se desarrolla en el material suplementario. El problema 25 retoma estos métodos en modelos de crecimiento.
	\end{quote}
	...
	\begin{enumerate}[label=\arabic*.]
		...
		\item si se consulta el campo de direcciones desarrollado en el material suplementario, ¿qué indica sobre las regiones de crecimiento y decrecimiento?
		\item ¿la ecuación es autónoma? Si lo es, ¿cuáles son sus equilibrios y qué estabilidad tienen?
		\item después del análisis cualitativo, ¿puede resolverse por separación de variables o corresponde al caso de integración directa desarrollado en el material suplementario?
